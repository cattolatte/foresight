...
A review of this project's own results, prompted by a number that should have
been suspicious from the start: the world model scored 0.783 AUC against 0.761
for a logistic regression on the same features. Three hundred thousand
parameters, an LSTM, an attention layer and a learned dynamics model, for 0.022
AUC over a linear classifier. Either the model was doing very little, or the
task was not asking for much.

It was the task. This document works out how, what survives the scrutiny, and
what to change.

---

## 1. The bar nobody had drawn

A score is only evidence if something simpler cannot produce it. So: what can
beat this benchmark *without a model*? (`eval/shortcuts.py`, test days, forecast
gap of four windows.)
...
| one feature — fraction carrying the `-1` window sentinel | 0.701 | 0.498 |
| always predict attack | 0.500 | 0.410 |
| *world model, for comparison* | *0.783* | *0.557* |

**A single feature — flow count — matches the world model to three decimals.**
Persistence beats it by nearly nine points. The headline result is not evidence
that anything was learned about attack dynamics.

Persistence is not a deployable system — it needs the answer in order to give
it. It is a measurement of how much of the task is autocorrelation, and the
answer is *most of it*.

## 2. Why persistence wins: the horizon is shorter than the attacks

Attack episodes in this capture are long. Median duration is 60s on Thursday and
240s on Friday, with the longest running seventy minutes. The forecast covers 90
seconds of traffic beginning a minute after the last observation. "Is there an
attack in the next minute and a half" is therefore very nearly "is there an
attack right now", and the benchmark is graded largely on the autocorrelation of
the label.
...
already observed, half of it, and `t+2` a quarter. Two of six horizon steps were
detection wearing a forecast's name. `SequenceSet` now takes a `gap`, defaulting
to `window/stride`, so the first labelled window is the first one the history
has not seen.

Correcting it moved AUC 0.790 → 0.785 on the model of the day (before the
feature fixes in §3, which is why neither figure is the 0.783 quoted elsewhere),
and *that* is the finding: the leak was not what held the number up.
Autocorrelation was.

## 3. Why TCP-ness wins: every attack here is TCP

`Init_Win_bytes_forward` is `-1` for 35% of flows. It is not a value; it is a
"not applicable" sentinel, set on every UDP and non-TCP flow and never on a TCP
one. And **44% of benign flows carry it against 0.0% of every attack family**,
...
a small negative quantity, so window means silently blended "how much TCP" into
"how large the TCP window was". It is now split into an explicit absence rate
and a mean over the flows where the field applies.

## 4. The question that is actually hard

Restrict to windows whose entire observed history is benign, and ask whether an
attack *begins*. Persistence is undefined here by construction — every input is
quiet — and the ranking changes:
...
83 onset-positive windows — roughly fourteen attack episodes.** No model of this
size fits fourteen examples. That is not a tuning problem; it is the ceiling.

## 5. What does work: surprise, and what it proves
...
change, score the size of the movement. Traffic is bursty, and a model that
cannot beat "something moved" has learned nothing about how networks evolve.

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="figures/surprise-dark.png">
  <img alt="Onset-task AUC: world model surprise 0.710 unsupervised, logistic regression 0.751 supervised, naive dynamics control 0.577" src="figures/surprise-light.png">
</picture>

| signal | AUC (all windows) | AUC (onset only) |
|---|---|---|
...
correction and a retrain that reversed to 0.710 against 0.751. A gap of a
hundredth of an AUC across a reseed was never worth a claim in the first place.
The defensible statement is the one against the naive control, where the margin
is 0.20 rather than 0.01.

## 6. The mechanism behind every failure: aggregation
...
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="figures/snr-dark.png">
  <img alt="An attack's share of flows during its own episode, network-wide against on the host it touches: Bot 85x, web brute force 19x, XSS 11x, Infiltration 2x, PortScan 2x, DDoS 1x" src="figures/snr-light.png">
</picture>
...
## 7. Modelling the host instead of the network
...
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="figures/perhost-dark.png">
  <img alt="Detection rate per family, network-wide windows against per-host windows, at a matched ten percent false-alarm rate" src="figures/perhost-light.png">
</picture>
...
a distributed flood is a network-wide phenomenon, and splitting it across fifteen
hosts is the wrong description of it.

Folding the two views into one score does not work: taking the per-window
maximum over hosts scores 0.730 AUC against 0.752 for the network view alone,
because the maximum over fifteen hosts tracks the noisiest host rather than the
compromised one. That is the correct lesson rather than a failed experiment.
**Per-host alerts should stay per-host.** "Host .17 is behaving unlike itself"
is both the finer signal and the more useful sentence than any single network
...
ten capture days, multiple victim hosts and many more distinct episodes, on the
same feature schema. Nothing else on this list will move the numbers as much.
...
## 9. A correction to this document's own arithmetic

The AUC these numbers are computed with was wrong for tied scores. It ranked by
`argsort` alone, which hands equal scores arbitrary distinct ranks instead of the
average rank they share — and the baselines in §1 are precisely the tied cases:
persistence takes two values, always-positive takes one. It scored a coin-flip
binary predictor at 0.250 and a constant score at 0.250, both of which are 0.500.
...
unaffected, having no ties. The conclusion is unchanged and slightly firmer.
`_auc` is now checked against a brute-force pair count over randomised tie-heavy
...
One result does support a weaker and more defensible version of that claim —
benign-trained surprise beating naive dynamics 0.752 to 0.551, with no attack
label anywhere in the objective. And one measurement explains every remaining
failure: network-wide averaging costs 85× of signal on the attacks that matter.

Both belong in the submission. A project that reports the shortcuts its own
benchmark admits is worth more than one that reports 0.99 F1 and has not looked.
